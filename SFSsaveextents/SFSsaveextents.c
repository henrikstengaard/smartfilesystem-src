...
void saveextents(ULONG firstblock, BPTR fh);
...
LONG main() {
  ...
  UBYTE template[]="DEVICE=DRIVE/A,FIRSTDATABLOCK/A/N,OUTPUTFILE/A\n";

  struct {char *device;
          ULONG *firstdatablock;
          char *outputfile;} arglist={NULL};
            ...
            if((ioreq=CreateIORequest(msgport,sizeof(struct IOStdReq)))!=0) {
              if(OpenDevice((UBYTE *)BADDR(fssm->fssm_Device)+1,fssm->fssm_Unit,(struct IORequest *)ioreq,fssm->fssm_Flags)==0) {
                ...
                if((buffer=AllocVec(bytes_block,0))!=0) {
                  ...
                  if((fh=Open(arglist.outputfile, MODE_NEWFILE))!=0) {
                    saveextents(*arglist.firstdatablock, fh);
                    Close(fh);
                  }
                  else {
                    PrintFault(IoErr(), "error while opening outputfile");
                  }
                }
                else {
...
void saveextents(ULONG firstblock, BPTR fh) {
  ...
  while(firstblock!=0 && (errorcode=findbnode(6, firstblock, (struct BNode **)&ebn))==0) {
    block=firstblock;
    blocks=ebn->blocks;
    firstblock=ebn->next;

    printf("Found extent, block = %ld, blocks = %ld, next = %ld\n", block, blocks, firstblock);

    while(blocks-->0) {
      read(block++);
      Write(fh, buffer, bytes_block);
    }
  }

  if(errorcode!=0) {
    printf("Error %ld occured while trying to save the extents.\n",errorcode);
  }
}
