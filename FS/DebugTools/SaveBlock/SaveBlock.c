...
#include "work:fs12-15-nov-1998/deviceio.h"
...
LONG main() {
  ...
  UBYTE template[]="DEVICE=DRIVE/A,BLOCK/N/A,INPUTFILE/A,FIXCHECKSUM/S,FIXOWNBLOCK/S\n";

  struct {char *device;
          ULONG *block;
          char *inputfile;
          ULONG fixchecksum;
          ULONG fixownblock;} arglist={NULL};
...
LONG write(ULONG block) {
  return(safedoio(DIO_WRITE,buffer,1,block));
...
void setchecksum(void *buffer) {
  ...
  bh->checksum=-CALCCHECKSUM(bytes_block, buffer);
}
