...
void END(UBYTE *name) {
  ...
  diff=ecv2.ev_lo-ecv.ev_lo;
  if(diff<0) {
    diff=-diff;
  }

  DEBUG(("%s: ticks %ld (f=%ld)\n",name,diff,freq));
}
    ...
    void debug(UBYTE *fmt, ... ) {
      ...
      if((port=FindPort("SFS debug output port"))!=0) {
        if((msg=AllocVec(sizeof(struct Message)+512,MEMF_CLEAR))!=0) {
          ...
          RawDoFmt(fmt,args,(void (*)())"\x16\xC0\x4E\x75",dest);
    ...
    void debug(UBYTE *fmt, ... ) {
      ...
      RawDoFmt(fmt,args,(void (*)())"\x16\xC0\x4E\x75",serbuffer);
...
void xkprintf(ULONG type,const char *fmt, ... ) {
  ...
  if((debugdetailed & type)!=0 || ((type & 1)==0 && (debug & type)!=0)) {
    if((str=AllocVec(512,MEMF_CLEAR))!=0) {
      ...
      RawDoFmt((char *)fmt,args,(void (*)())"\x16\xC0\x4E\x75",dest);
      kprintf("%s",dest);
    }
  }
}
