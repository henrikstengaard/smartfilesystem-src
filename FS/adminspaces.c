...
extern ULONG block_adminspace;           /* the block offset of the first adminspacecontainer block */
...
LONG allocadminspace(struct CacheBuffer **returned_cb) {
  ...
  BLCK adminspaceblock=block_adminspace;
  LONG errorcode;
  ...
  while((errorcode=readcachebuffercheck(&cb,adminspaceblock,ADMINSPACECONTAINER_ID))==0) {
    ...
    LONG adminspaces=(bytes_block-sizeof(struct fsAdminSpaceContainer))/sizeof(struct fsAdminSpace);
    ...
    if((adminspaceblock=asc->next)==0) {
      ...
      adminspaceblock=block_adminspace;
...
LONG freeadminspace(BLCK block) {
  ...
  DEBUG(("freeadminspace: Entry -- freeing block %ld\n",block));
  ...
  while((errorcode=readcachebuffercheck(&cb,adminspaceblock,ADMINSPACECONTAINER_ID))==0) {
    ...
    while(adminspaces-->0) {
      if(block>=as->space && block<as->space+32) {
        ...
        DEBUG(("freeadminspace: Block to be freed is located in AdminSpaceContainer block at %ld\n",adminspaceblock));
