...
LONG getparentbtreecontainer(BLCK rootblock,struct CacheBuffer **io_cb) {
  ...
  XDEBUG((DEBUG_NODES,"getparentbtreecontainer: Getting parent of block %ld\n",(*io_cb)->blckno));
...
LONG createbnode(BLCK rootblock,ULONG key,struct CacheBuffer **returned_cb,struct BNode **returned_bnode) {
  ...
  XDEBUG((DEBUG_NODES,"createbnode: Creating BNode with key %ld, using %ld as root.\n",key,rootblock));

  while((errorcode=findbnode(rootblock, key, returned_cb, returned_bnode))==0) {
    struct fsBNodeContainer *bnc=(*returned_cb)->data;
    struct BTreeContainer *btc=&bnc->btc;
    LONG extbranches=(bytes_block-sizeof(struct fsBNodeContainer))/btc->nodesize;
    ...
    XDEBUG((DEBUG_NODES,"createbnode: findbnode found block %ld\n",(*returned_cb)->blckno));

    if(btc->nodecount<extbranches) {
      /* Simply insert new node in this BTreeContainer */

      XDEBUG((DEBUG_NODES,"createbnode: Simple insert\n"));
...
LONG splitbtreecontainer(BLCK rootblock,struct CacheBuffer *cb) {
  ...
  XDEBUG((DEBUG_NODES,"splitbtreecontainer: splitting block %ld\n",cb->blckno));
  ...
  if((errorcode=getparentbtreecontainer(rootblock,&cbparent))==0) {
    if(cbparent==0) {
      ...
      XDEBUG((DEBUG_NODES,"splitbtreecontainer: creating root tree-container\n"));
      ...
      if((errorcode=allocadminspace(&cb))==0) {
        ...
        CopyMemQuick(cbparent->data,cb->data,bytes_block);
        bnc->bheader.ownblock=cb->blckno;

        XDEBUG((DEBUG_NODES,"splitbtreecontainer: allocated admin space for root\n"));
    ...
    if(errorcode==0) {
      struct fsBNodeContainer *bncparent=cbparent->data;
      struct BTreeContainer *btcparent=&bncparent->btc;
      LONG branches=(bytes_block-sizeof(struct fsBNodeContainer))/btcparent->nodesize;
      ...
      if(errorcode==0) {
        ...
        if((errorcode=allocadminspace(&cbnew))==0) {
          ...
          struct fsBNodeContainer *bnc=cb->data;
          struct BTreeContainer *btc=&bnc->btc;
          LONG branches=(bytes_block-sizeof(struct fsBNodeContainer))/btc->nodesize;
          ULONG newkey;
...
LONG findbnode(BLCK rootblock, ULONG key, struct CacheBuffer **returned_cb, struct BNode **returned_bnode) {
  ...
  XDEBUG((DEBUG_NODES,"findbnode: Looking for BNode with key %ld, from root %ld\n",key,rootblock));
  ...
  XDEBUG((DEBUG_NODES,"findbnode: *returned_cb->blckno = %ld, Exiting with errorcode %ld\n",(*returned_cb)->blckno,errorcode));
...
LONG deleteinternalnode(BLCK rootblock,struct CacheBuffer *cb,ULONG key) {
  struct fsBNodeContainer *bnc=cb->data;
  struct BTreeContainer *btc=&bnc->btc;
  UWORD branches=(bytes_block-sizeof(struct fsBNodeContainer))/btc->nodesize;
  LONG errorcode;
  ...
  if((errorcode=storecachebuffer(cb))==0) {
    ...
    XDEBUG((DEBUG_NODES,"deleteinternalnode: branches = %ld, btc->nodecount = %ld\n",branches,btc->nodecount));
    ...
    if(btc->nodecount<(branches+1)/2) {
      ...
      if((errorcode=getparentbtreecontainer(rootblock,&cbparent))==0) {
        if(cbparent!=0) {
          ...
          XDEBUG((DEBUG_NODES,"deleteinternalnode: get parent returned block %ld.\n",cbparent->blckno));
          ...
          if(n<btcparent->nodecount-1) {      // Check if we have a next neighbour.
            ...
            XDEBUG((DEBUG_NODES,"deleteinternalnode: using next container.\n"));
            ...
            XDEBUG((DEBUG_NODES,"deleteinternalnode: using prev container.\n"));
          ...
          */
          ...
          XDEBUG((DEBUG_NODES,"deleteinternalnode: no parent so must be root\n"));
          ...
          if(btc->isleaf==FALSE) {
            ...
            if((errorcode=readcachebuffercheck(&cb2,btc->bnode[0].data,BNODECONTAINER_ID))==0) {
              ...
              CopyMemQuick(cb2->data,cb->data,bytes_block);
              bnc->bheader.ownblock=cb->blckno;
        ...
        XDEBUG((DEBUG_NODES,"deleteinternalnode: almost done\n"));
...
LONG deletebnode(BLCK rootblock,ULONG key) {
  ...
  if((errorcode=findbnode(rootblock,key,&cb,&bn))==0) {
    ...
    XDEBUG((DEBUG_NODES,"deletebnode: key %ld\n",key));
...
void removebnode(ULONG key,struct BTreeContainer *btc) {
  ...
  XDEBUG((DEBUG_NODES,"removebnode: key %ld\n",key));
