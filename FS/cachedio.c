...
extern ULONG __asm BMTSTO(register __a0 ULONG *, register __d0 ULONG, register __d1 ULONG);
extern ULONG __asm BMTSTZ(register __a0 ULONG *, register __d0 ULONG, register __d1 ULONG);

extern ULONG __asm BMCNTO(register __a0 ULONG *, register __d0 ULONG, register __d1 ULONG);

extern LONG bmclr(ULONG *,LONG,LONG,LONG);
extern LONG bmset(ULONG *,LONG,LONG,LONG); //  bitmap, longs, bitoffset, bits
...
BYTE iocache_readonwrite=FALSE;       /* Determines whether a new line is read before writing to it. */
...
LONG setiocache(ULONG lines, ULONG readahead, BYTE copyback) {
  ...
  if((lruhead=allocate(sizeinblocks<<shifts_block, lines))!=0) {
    ...
    if(iocache_lruhead==0 || (errorcode=flushiocache())==0) {
      ...
      if(iocache_readonwrite==FALSE && iocache_copyback!=FALSE) {
        ioc_buffer=(struct IOCache *)iocache_lruhead->mlh_Head;
        ioc_buffer->locked=TRUE;
      }
...
LONG validateiocache(struct IOCache *ioc, ULONG blockoffset, ULONG blocks) {
  ...
  if(iocache_readonwrite!=FALSE || BMTSTO(ioc->valid, blockoffset, blocks)!=0) {
    return(0);
  }

//  DEBUG(("validateiocache: ioc->block = %ld, ioc->blocks = %ld, ioc->dirty = 0x%08lx, ioc->valid = 0x%08lx\n", ioc->block, ioc->blocks, ioc->dirty[0], ioc->valid[0]));

  if((errorcode=transfer(DIO_READ, ioc_buffer->data, ioc->block, ioc->blocks))==0) {
    LONG i=iocache_sizeinblocks;

//    DEBUG(("validateiocache: BMCNTO returned %ld\n", BMCNTO(ioc->dirty, 0, ioc->blocks)));

    if(BMCNTO(ioc->dirty, 0, ioc->blocks) < iocache_sizeinblocks/2) {
      void *data;

      /* Copying the dirty blocks to the new IOCache. */

//      DEBUG(("validateiocache: Using new IOCache\n"));

      while(--i>=0) {
        if(BMTSTO(ioc->dirty, i, 1)!=FALSE) {
          CopyMemQuick((UBYTE *)ioc->data + (i<<shifts_block), (UBYTE *)ioc_buffer->data + (i<<shifts_block), bytes_block);
        }
      }

      data=ioc_buffer->data;
      ioc_buffer->data=ioc->data;
      ioc->data=data;
    }
    else {
...
//      DEBUG(("validateiocache: Using existing IOCache\n"));

      while(--i>=0) {
        if(BMTSTZ(ioc->dirty, i, 1)!=FALSE) {
          CopyMemQuick((UBYTE *)ioc_buffer->data + (i<<shifts_block), (UBYTE *)ioc->data + (i<<shifts_block), bytes_block);
        }
      }
    }
...
//      DEBUG(("copybackiocache: calling validateiocache\n"));
      if((errorcode=validateiocache(ioc, 0, ioc->blocks))!=0) {
        break;
      }
    }

    if((errorcode=transfer(DIO_WRITE, (UBYTE *)ioc->data + (dirtylow<<shifts_block), ioc->block + dirtylow, dirtyhigh - dirtylow + 1))!=0) {
      break;
    }
...
LONG readonwriteintocache(BLCK block, struct IOCache **returned_ioc) {
  ...
  if((ioc=locateiocache(block))==0) {
    if((errorcode=lruiocache(&ioc))==0) {
      ...
      if(iocache_readonwrite==FALSE || (errorcode=transfer(DIO_READ, ioc->data, blockstart, blocklength))==0) {
        ioc->block=blockstart;
        ioc->blocks=blocklength;

        if(iocache_readonwrite!=FALSE) {
          ioc->valid[0]=0xFFFFFFFF;
          ioc->valid[1]=0xFFFFFFFF;
          ioc->valid[2]=0xFFFFFFFF;
          ioc->valid[3]=0xFFFFFFFF;
...
//      DEBUG(("IOCACHE: found overlapping cache (%ld-%ld) for block %ld of %ld blocks\n",ioc->block,ioc->block+ioc->blocks-1,block,blocks));
      ...
      src=buffer + ((startinline-block)<<shifts_block);
      dst=(UBYTE *)ioc->data + (offsetinline<<shifts_block);

      if(((ULONG)buffer & 0x00000003) != 0) {
        CopyMem(src, dst, overlappedblocks<<shifts_block);
      }
      else {
        CopyMemQuick(src, dst, overlappedblocks<<shifts_block);
      }
    }
      ...
      if((errorcode=readonwriteintocache(block, &ioc))==0) {              /* a trick, which works because cachesystem consists of atleast 4 blocks. */
        if((errorcode=readonwriteintocache(block+blocks-1, &ioc2))==0) {
          ...
          if(ioc==ioc2) {
            ...
            LONG blocks2=iocache_sizeinblocks-offsetinline;
