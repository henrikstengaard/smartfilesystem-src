...
extern struct fsStatistics statistics;

extern void __asm AsmFreePooled(register __a0 void *,
                         register __a1 void *,
                         register __d0 ULONG,
                         register __a6 struct ExecBase *);

extern void __asm AsmDeletePool(register __a0 void *,
                         register __a6 struct ExecBase *);

extern void * __asm AsmCreatePool(register __d0 ULONG,
                           register __d1 ULONG,
                           register __d2 ULONG,
                           register __a6 struct ExecBase *);

extern void * __asm AsmAllocPooled(register __a0 void *,
                            register __d0 ULONG,
                            register __a6 struct ExecBase *);

void *transactionpool=0;
ULONG transactionpoolsize;
UBYTE *compressbuffer=0;
LONG transactionnestcount=0;
...
#define ROOT     operationroot       /* The name of the root variable */
#define RBNODE   struct Operation    /* The name of the structure containing the left,
                                        right, parent, nodecolor & data fields. */
#define SENTINEL operationsentinel   /* The name of the sentinel node variable (must be a RBNODE) */
...
void removeoperation(struct Operation *o) {
  ...
  transactionpoolsize-=o->oi.length+sizeof(struct Operation);
  AsmFreePooled(transactionpool,o,o->oi.length+sizeof(struct Operation), SysBase);
}
...
LONG inittransactions() {
  ...
  poolsize=bytes_block*4;
  if(poolsize<16384) {
    poolsize=16384;
  }

  if((compressbuffer=AllocMem(bytes_block+(bytes_block>>4),1))!=0) {
    if((transactionpool=AsmCreatePool(0,poolsize,poolsize>>1, SysBase))!=0) {
      return(0);
    }
  }
...
void cleanuptransactions() {
  if(transactionpool!=0) {
    AsmDeletePool(transactionpool, SysBase);
  }

  if(compressbuffer!=0) {
    FreeMem(compressbuffer,bytes_block+bytes_block>>4);
  }
}
...
LONG fillwithoperations(struct fsTransactionStorage *ts,struct Operation **o,UBYTE **src,LONG *length) {
  LONG bytesleft=bytes_block-sizeof(struct fsTransactionStorage);
  LONG copylength;
  UBYTE *dest=ts->data;
...
LONG savetransaction(BLCK *firsttransactionblock) {
  ...
  XDEBUG((DEBUG_TRANSACTION,"savetransaction: Entry.  Transaction size = %ld\n", transactionpoolsize));
  ...
  *firsttransactionblock=0;
  ...
  if((cb=getcachebuffer())!=0) {
    ...
    if((errorcode=findspace(1, block_rovingblockptr, block_rovingblockptr, &blckno))==0) {
      ULONG startblock;

      XDEBUG((DEBUG_TRANSACTION,"savetransaction: findspace succesfully completed\n"));
      ...
      do {
        ...
        XDEBUG((DEBUG_TRANSACTION,"savetransaction: Storing operations of transaction in block %ld\n",blckno));
        ...
        if(done==FALSE) {
          if(startblock==block_rovingblockptr) {
            errorcode=ERROR_DISK_FULL;
            break;
          }

          if((errorcode=findspace(1, startblock, block_rovingblockptr, &blckno))!=0) {
            break;
          }
  ...
  XDEBUG((DEBUG_TRANSACTION,"savetransaction: Exiting with errorcode = %ld\n",errorcode));
...
LONG loadtransaction(BLCK block) {
  ...
  while(block!=0 && (errorcode=readcachebuffercheck(&cb,block,TRANSACTIONSTORAGE_ID))==0) {
    ...
    bytesleft=bytes_block-sizeof(struct fsTransactionStorage);
    src=ts->data;
    ...
    while(bytesleft!=0) {
      if(length!=0) {
        ...
        if((o=AsmAllocPooled(transactionpool,len+sizeof(struct Operation), SysBase))==0) {
          return(ERROR_NO_FREE_STORE);
        }
        transactionpoolsize+=len+sizeof(struct Operation);
...
LONG checkfortransaction(void) {
  ...
  if((errorcode=readcachebuffer(&cb,block_root+2))==0) {
    struct fsBlockHeader *bh=cb->data;

    if(checkchecksum(cb)==DOSTRUE && bh->ownblock==block_root+2) {
...
//                                           "Ok",((UBYTE *)BADDR(devnode->dn_Name))+1);
            ...
            DEBUG(("Before removetransactionfailure()\n"));
...
void combineoperations(void) {
  ...
  if((o=FirstNode())!=0) {
    ...
    BLCK block;
    UBYTE tnc=transactionnestcount-1;
...
LONG addoperation2(struct CacheBuffer *cb_org, struct CacheBuffer *cb_new) {
  ...
  // DEBUG(("addoperation: Adding a new operation for block %ld\n",block));
  ...
  if((o=AsmAllocPooled(transactionpool,bytes_block+sizeof(struct Operation), SysBase))!=0) {
    UBYTE bits=0;
...
LONG addoperation(BLCK block,UBYTE *data,UWORD length,UBYTE bits) {
  ...
  if((o=AsmAllocPooled(transactionpool, length+sizeof(struct Operation), SysBase))!=0) {
    transactionpoolsize+=length+sizeof(struct Operation);
...
LONG addfreeoperation(BLCK block) {
  ...
  if((o=AsmAllocPooled(transactionpool, sizeof(struct Operation), SysBase))!=0) {
    transactionpoolsize+=sizeof(struct Operation);
...
void newtransaction(void) {
  ...
  XDEBUG((DEBUG_TRANSACTION,"--NEW-----> poolsize = %ld\n",transactionpoolsize));
}
...
void endtransaction(void) {
  ...
  XDEBUG((DEBUG_TRANSACTION,"--END----->\n"));

  combineoperations();

  if(--transactionnestcount==0) {  /* Important!  Combineoperations() must be called before lowering transactionnestcount */
    ...
    if(transactionpoolsize>MAX_TRANSACTIONPOOLSIZE) {
      TDEBUG(("endtransaction: poolsize larger than 32 kB -> flushed transaction\n"));
      flushtransaction();
    }
  }
}
...
void deletetransaction(void) {
  ...
  while(o!=0) {
    ...
    if(oprev->new==transactionnestcount) {
      struct CacheBuffer *cb;
...
//    DEBUG(("writeoperations: Writing block %ld\n",o->oi.blckno));
//            request(PROGRAMNAME " request","%s\n"\
//                                           "Writing block %ld\n",
...
LONG flushtransaction(void) {
  ...
  XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Entry\n"));
  ...
  if(errorcode==0 && hastransaction()) {
    ...
    DEBUG(("flushtransaction: There is a transaction\n"));
    ...
    if(errorcode==0) {
      ...
      XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Saved transaction\n"));

      while((errorcode=flushiocache())!=0 && flusherror(errorcode)==1) {     /* This commits any dirty data. */
      }

      XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Disk updated\n"));
      ...
      if(errorcode==0) {
        if((cb=getcachebuffer())!=0) {
          ...
          tf->bheader.ownblock=block_root+2;
          ...
          XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Set TransactionFailure block\n"));
          ...
          if(errorcode==0) {
            ...
            XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Disk updated (2)\n"));

            if(errorcode==0) {
              if((errorcode=writeoperations())==0) {           /* writeoperations() */

                XDEBUG((DEBUG_TRANSACTION,"flushtransaction: Updated all blocks\n"));
  ...
  DEBUG(("flushtransaction: Done.  errorcode = %ld\n",errorcode));
...
struct Operation *getlatestoperation(BLCK block) {
  ...
  XDEBUG((DEBUG_CACHEBUFFER,"getlatestoperation: Entry for block %ld\n",block));

  o=getlatestoperation2(block, ROOT);

  XDEBUG((DEBUG_CACHEBUFFER,"getlatestoperation: Exit\n"));
...
void restorecachebuffer(struct CacheBuffer *cb) {
  ...
  XDEBUG((DEBUG_CACHEBUFFER,"restorecachebuffer: Entry.  cb->blckno = %ld, cb->bits = %ld\n",cb->blckno, (ULONG)cb->bits));
...
LONG applyoperation(BLCK block, struct CacheBuffer **returned_cb) {
  ...
  if((o=getlatestoperation(block))!=0) {
    ...
    statistics.cache_operationdecode++;

    XDEBUG((DEBUG_CACHEBUFFER,"applyoperation: Entry. block = %ld -> found an operation!\n",block));

    if((o->oi.bits & OI_EMPTY)!=0) {
      statistics.cache_emptyoperationdecode++;
    ...
    *returned_cb=cb;
    ...
    XDEBUG((DEBUG_CACHEBUFFER,"applyoperation: Entry. block = %ld -> returning original!\n",block));
...
ULONG transactionspace(void) {
  UWORD space=(bytes_block-sizeof(struct fsTransactionStorage));
...
//  return((transactionpoolsize+bytes_block+(bytes_block>>6)+space-1)/space);
  ...
  return((transactionpoolsize + space-1)/space);
}
