...
extern ULONG node_containers;     /* number of containers per ExtentIndexContainer */
extern ULONG bytes_block;           /* size of a block in bytes */
extern UWORD shifts_block32;        /* shift count needed to convert a blockoffset<->32byteoffset */
  ...
  while(noderoot!=childblock && (errorcode=readcachebuffercheck(io_cb, noderoot, NODECONTAINER_ID))==0) {
    ...
    if(nc->nodes==1) {
      ...
      *io_cb=0;
      ...
      noderoot=nc->node[containerentry]>>shifts_block32;
    }
  }
...
LONG freecontainer(BLCK noderoot, struct CacheBuffer *cb) {
  ...
  if((errorcode=parentnodecontainer(noderoot, &cb))==0 && cb!=0) {   /* This line also prevents the freeing of the noderoot. */
    ...
    if((errorcode=freeadminspace(nc->node[containerindex]>>shifts_block32))==0) {
...
LONG addnewnodelevel(BLCK noderoot, UWORD nodesize) {
  ...
  XDEBUG((DEBUG_NODES,"addnewnodelevel: Entry\n"));
  ...
  if((errorcode=readcachebuffercheck(&cb, noderoot, NODECONTAINER_ID))==0) {
    ...
    if((errorcode=allocadminspace(&newcb))==0) {
      ...
      CopyMemQuick(nc->node, newnc->node, bytes_block-sizeof(struct fsNodeContainer));
      ...
      if((errorcode=storecachebuffer(newcb))==0) {
        ...
        if(nc->nodes==1) {
          nc->nodes=(bytes_block-sizeof(struct fsNodeContainer))/nodesize;
        }
        else {
          nc->nodes*=node_containers;
        }

        nc->node[0]=(newblock<<shifts_block32)+1;  /* Tree is full from that point! */

        ClearMemQuick(&nc->node[1], bytes_block-sizeof(struct fsNodeContainer)-4);
  ...
  XDEBUG((DEBUG_NODES,"addnewnodelevel: Exiting with errorcode %ld\n",errorcode));
...
LONG deletenode(BLCK noderoot, struct CacheBuffer *cb, struct fsNode *n, UWORD nodesize) {
  struct fsNodeContainer *nc=cb->data;
  UWORD nodecount=(bytes_block-sizeof(struct fsNodeContainer))/nodesize;
  WORD i=nodecount;
  WORD empty=0;
  LONG errorcode;
...
LONG createnodecontainer(ULONG nodenumber, ULONG nodes, BLCK *returned_block) {
  ...
  XDEBUG((DEBUG_NODES,"createnodecontainer: nodenumber = %ld, nodes = %ld\n",nodenumber,nodes));
  ...
  XDEBUG((DEBUG_NODES,"createnodecontainer: Exiting with errorcode %ld\n",errorcode));
...
//  XDEBUG((DEBUG_NODES,"createnode: Entry\n"));
    ...
    if(nc->nodes==1) {    // Is it a leaf-container?
      ...
      if(i>=0) {
        ...
        *returned_nodeno=nc->nodenumber+((UBYTE *)n-(UBYTE *)nc->node)/nodesize;
        ...
        XDEBUG((DEBUG_NODES,"createnode: Created Node %ld\n",*returned_nodeno));
      ...
      if(i>=0) {
        ...
        if(i>=0) {
          ...
          if(nc->nodes==(bytes_block-sizeof(struct fsNodeContainer))/nodesize) {
            nodes=1;
          }
          else {
            nodes=nc->nodes/node_containers;
          }
...
//  XDEBUG((DEBUG_NODES,"createnode: Exiting with errorcode %ld\n",errorcode));
...
LONG findnode(BLCK nodeindex,UWORD nodesize,NODE nodeno,struct CacheBuffer **returned_cb,struct fsNode **returned_node) {
  ...
  XDEBUG((DEBUG_NODES,"findnode: Entry -- looking for nodeno %ld\n",nodeno));
  ...
  while((errorcode=readcachebuffercheck(&cb,nodeindex,NODECONTAINER_ID))==0) {
    ...
    XDEBUG((DDEBUG_NODES,"findnode: Read NodeContainer at block %ld with nodenumber = %ld, nodes = %ld\n",nodeindex,nc->nodenumber,nc->nodes));
  ...
  XDEBUG((DEBUG_NODES,"findnode: Exiting with errorcode %ld\n",errorcode));
