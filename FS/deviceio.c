...
LONG transfer_buffered(UWORD action, UBYTE *buffer, ULONG blockoffset, ULONG blocklength) {
  ...
  while(blocklength!=0) {
    ...
    setiorequest(&fsioreq, action, tempbuffer, blockoffset, blocks);
    ...
    while((errorcode=DoIO((struct IORequest *)fsioreq.ioreq))!=0) {
...
LONG waittransfers(void) {
  ...
  while(iolist!=0) {
    struct fsIORequest *next=iolist->next;

    if((errorcode=WaitIO((struct IORequest *)iolist->ioreq))!=0 && firsterrorcode==0) {
      while((errorcode=handleioerror(errorcode, iolist->action))==0) {
        if((errorcode=DoIO((struct IORequest *)iolist->ioreq))==0) {
          break;
        }
      }
    ...
    FreeMem(iolist, sizeof(struct fsIORequest));
    iolist=next;
  }
...
LONG asynctransfer(UWORD action, UBYTE *buffer, ULONG blockoffset, ULONG blocklength) {
  ...
  if(blockoffset < blocks_total && blockoffset+blocklength <= blocks_total) {
    ULONG maxblocks=blocks_maxtransfer;

    if(((ULONG)buffer & ~mask_mask)!=0) {   // Buffered transfer needed?
...
LONG transfer(UWORD action, UBYTE *buffer, ULONG blockoffset, ULONG blocklength) {
  ...
  TDEBUG(("TRANSFER: %ld, buf=0x%08lx, block=%ld, blocks=%ld -", action, buffer, blockoffset, blocklength));
  ...
  if(blockoffset < blocks_total && blockoffset+blocklength <= blocks_total) {
    ...
    while(blocklength!=0) {
      ...
      setiorequest(&fsioreq, action, buffer, blockoffset, blocks);
