...
static const char version[]={"\0$VER: SFSsaveblock 1.0 " __AMIGADATE__ "\r\n"};
...
LONG main() {
  ...
  UBYTE template[]="DEVICE=DRIVE/A,BLOCK/N/A,INPUTFILE/A,FIXCHECKSUM/S,FIXOWNBLOCK/S,WRITE/S\n";
        ...
        if((dl=FindDosEntry(dl,arglist.device,LDF_DEVICES))!=0) {
          ...
          if((initcachedio((UBYTE *)BADDR(fssm->fssm_Device)+1, fssm->fssm_Unit, fssm->fssm_Flags, dosenvec))==0) {
            ...
            if((buffer=AllocVec(bytes_block, bufmemtype))!=0) {
              ...
              if((fh=Open(arglist.inputfile, MODE_OLDFILE))!=0) {
                LONG bytesread=Read(fh,buffer,bytes_block);
                LONG bytesread2=Read(fh,buffer,1);
                Close(fh);

                if(bytesread==bytes_block && bytesread2==0) {
                  if(checkchecksum(buffer)!=DOSTRUE) {
                    printf("Warning: Checksum of the read block is wrong.  It is 0x%08lx while it should be 0x%08lx.\n", ((struct fsBlockHeader *)buffer)->checksum, calculatechecksum(buffer));
                  }
                  if(((struct fsBlockHeader *)buffer)->ownblock != *arglist.block) {
                    printf("Warning: The read block indicates that it is actually block %ld, not block %ld. (ownblock is wrong!)\n", ((struct fsBlockHeader *)buffer)->ownblock, *arglist.block);
                  }
                  if(arglist.fixownblock!=0) {
                    ((struct fsBlockHeader *)buffer)->ownblock=*arglist.block;
                    printf("Fixing ownblock number.\n");
                  }
                  if(arglist.fixchecksum!=0) {
                    setchecksum(buffer);
                    printf("Fixing checksum of block.  It is now: 0x%08lx\n", ((struct fsBlockHeader *)buffer)->checksum);
                  }

                  if(arglist.write!=0) {
                    write(*arglist.block, buffer, 1);
                    printf("Block was written\n");
                  }
                  else {
                    printf("Block was NOT written (use the WRITE option to really write a block)\n");
                  }
                }
                else {
                  printf("Input file couldn't be read or has the wrong size (must be %ld bytes)\n", bytes_block);
                }
